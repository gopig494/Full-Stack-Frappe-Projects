frappe.pages['test-info-menu'].on_page_load = function(wrapper) {
	var page = frappe.ui.make_app_page({
		parent: wrapper,
		title: 'Test Menu Info',
		single_column: true
	});
      $(frappe.render_template("test_info_meno")).appendTo(page.body.addClass("listing"));
...
frappe.pages['test-info-menu'].refresh = function(wrapper) {
    // location.reload();

    $('.page-title .title-text').text('Product Details')

    frappe.call({
        method: 'employee_management.employee_management.doctype.product_datatable.product_datatable.get_product',
        args: { dtype: "Product" },
        callback: function(r) {
...
function updatemenuitems() {
    ...
    for (var i = 0; i < datas.length; i++) {
        ...
        console.log(item);
        frappe.call({
            method: 'employee_management.employee_management.doctype.product_datatable.product_datatable.update_product',
            args: {
                name: itemid,
                title: itemTitle,
                sku: itemSKU,
